...
public class Day16B : Problem<Day16B.Input, int> {
    ...
    private Grid<DirectionElement> SpreadLight(Grid<TileElement> grid, (Int2 position, Direction direction) start) {
        Grid<DirectionElement> light = new(grid.Size);

        Stack<(Int2 position, Direction direction)> spreadStack = new();
        spreadStack.Push(start);
    ...
    private int SpreadAndCount(Grid<TileElement> grid, (Int2 position, Direction direction) start) {
        Grid<DirectionElement> light = SpreadLight(grid, start);
        return light.Positions().Count(p => light[p] != Direction.EMPTY);
    }

    protected override int Solve(Input input) {
        return Enumerable.Range(0, input.Grid.Size.X).Select(x => (new Int2(x, 0), Direction.NORTH))
                         .Concat(
                              Enumerable.Range(0, input.Grid.Size.X)
                                        .Select(x => (new Int2(x, input.Grid.Size.Y - 1), Direction.SOUTH))
                          )
                         .Concat(
                              Enumerable.Range(0, input.Grid.Size.Y)
                                        .Select(y => (new Int2(0, y), Direction.EAST))
                          )
                         .Concat(
                              Enumerable.Range(0, input.Grid.Size.Y)
                                        .Select(y => (new Int2(input.Grid.Size.X - 1, y), Direction.WEST))
                          )
                         .Max(start => SpreadAndCount(input.Grid, start));
    }
