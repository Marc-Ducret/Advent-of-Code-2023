...
public class Day11B : Problem<Day11B.Input, long> {
    public readonly record struct Input(Map Map);

    public readonly record struct Galaxy(Long2 Position);
    ...
    protected override long Solve(Input input) {
        ...
        {
            Int2       expansion       = 0;
            const long expansionFactor = 1_000_000 - 1;
            ...
            for (int y = 0; y < input.Map.Size.Y; y++) {
                ...
                for (int x = 0; x < input.Map.Size.X; x++) {
                    ...
                    if (input.Map[mapPosition] == Tile.GALAXY)
                        galaxies.Add(new Galaxy(mapPosition + expansionFactor * (Long2)expansion));
                }
            }
        }
        ...
        for (int j = i + 1; j < galaxies.Count; j++) {
            sum += Long2.CSum(Long2.Abs(galaxies[i].Position - galaxies[j].Position));
        }
